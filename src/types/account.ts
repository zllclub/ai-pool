...
export type QuotaRequestReason = 'manual' | 'manualAll' | 'automatic' | 'lowQuotaAutomatic' | 'appStartup' | 'widgetManual' | 'autoSwitchCheck' | 'import' | 'oauthLogin';
export interface QuotaRequestLog {
  timestamp: number; accountId: string; accountLabel: string;
  reason: QuotaRequestReason; success: boolean; errorCode: string | null;
}
export interface LoginResult { account: CodexAccount; quotaError: AppError | null }
export type AccountAction = 'refresh' | 'codex' | 'pi' | 'reauthorize' | 'delete';
