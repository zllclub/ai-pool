...
interface Props {
  account: CodexAccount; codex: boolean; pi: boolean; busy: boolean; refreshing: boolean; pinned: boolean;
  dragging: boolean; error?: AppError; now: number;
  onAction: (action: AccountAction, account: CodexAccount) => void;
  onPin: (account: CodexAccount) => void;
  onPointerDragStart: (accountId: string, event: PointerEvent<HTMLDivElement>) => void;
}

export const AccountCard = memo(function AccountCard({ account: a, codex, pi, busy, refreshing, pinned, dragging, error, now, onAction, onPin, onPointerDragStart }: Props) {
  ...
  return <div className={`account-card-sortable${dragging ? ' is-dragging' : ''}`} data-account-id={a.id}
    onPointerDown={event => {
      if (busy || event.button !== 0) return;
      const interactive = (event.target as HTMLElement).closest('button, a, input');
      if (interactive) return;
      onPointerDragStart(a.id, event);
    }}>
    <Card className={`account-card${codex || pi ? ' is-active' : ''}`} bordered>
    <header className="card-header">
      ...
      <div className="identity">
        ...
        <div className="identity-meta">{codex || pi ? <span className="account-state">{codex ? <CurrentAccountBadge name="Codex" /> : null}{pi ? <CurrentAccountBadge name="Pi Agent" /> : null}</span> : null}<span className="header-updated">{a.quota ? `更新于 ${new Date(a.quota.updatedAt).toLocaleTimeString()} · ${Math.max(0, Math.floor((now - a.quota.updatedAt) / 60_000))} 分钟前` : '尚未查询额度'}{busy ? ' · 处理中…' : ''}</span></div>
      </div>
